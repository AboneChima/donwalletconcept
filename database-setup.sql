-- Setup script for Don Wallet Construction Company Database
...
-- Step 3: Insert projects ONLY if they don't already exist (avoids duplicates)
INSERT INTO projects (slug, title, location, year, category, size, description, thumbnail, hero, images)
SELECT * FROM (VALUES
...
) AS v(slug, title, location, year, category, size, description, thumbnail, hero, images)
WHERE NOT EXISTS (
    SELECT 1 FROM projects WHERE projects.slug = v.slug
);

-- Done! You should see "X rows inserted" where X is the number of new projects added
-- If you see "0 rows inserted", it means the projects already exist (no duplicates!)
